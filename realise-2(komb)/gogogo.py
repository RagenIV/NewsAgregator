...
urlists = [{'https://news.yandex.ru/':'data-counter=".*">(.*?)</a></h2>',
...
while True:
    ...
    print('{0:_>2}|{1:_^16}|{2:_^13}'.format(" №", "слово", "перемещение"))
    # print('{:_<31}'.format(''))
    for i in range(kvo):
        t = ''
        if status[i][0] == '+':
            ...
            t = '\033[0m'
        print(t+('{0:2d}|{1:16}|{2:^13}'.format(i+1, ans[i].upper(), status[i]))+'\033[0m')
